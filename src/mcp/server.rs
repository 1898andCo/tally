...
pub fn read_resource_summary(store: &GitFindingsStore) -> Result<String, McpError> {
    ...
    recent.sort_by_key(|f| std::cmp::Reverse(f.created_at));
    recent.truncate(10);
...
pub fn read_resource_version(store: &GitFindingsStore) -> Result<String, McpError> {
    let rule_count =
        crate::registry::store::RuleStore::load_all_rules(store).map_or(0, |r| r.len());
...
pub fn read_resource_rules_summary(store: &GitFindingsStore) -> Result<String, McpError> {
    ...
    let mut rule_usage: Vec<(&str, u32)> = rules
        ...
        .collect();
    rule_usage.sort_by_key(|r| std::cmp::Reverse(r.1));
